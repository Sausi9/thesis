...
from src.distributions.targets import GaussianTargetMarginal, build_target_marginal
from src.jeffrey.update import jeffrey_updated_gaussian_params
from src.utils import resolve_path, timestamped_output_path


def looks_like_derived_sample(path: Path) -> bool:
    derived_markers = (
        "exact_jeffrey",
    ...
    return any(marker in path.stem for marker in derived_markers)
...
def find_latest_model_sample(samples_dir: Path) -> Path:
    ...
    candidates = [path for path in candidates if not looks_like_derived_sample(path)]
...
def make_output_path(cfg: DictConfig, project_root: Path, source_path: Path) -> Path:
    return timestamped_output_path(
        ...
        default_stem=f"{source_path.stem}_importance_resampling_samples",
        extension=".pt",
    )
...
def main(cfg: DictConfig) -> None:
    ...
    result = {
        "samples": resampled,
        "sample_type": "importance_resampling",
