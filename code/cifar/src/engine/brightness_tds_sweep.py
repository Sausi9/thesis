...
cache_root = Path(tempfile.gettempdir()) / "cifar_brightness_sweep_cache"
...
def make_sweep_dir(project_root: Path, cfg: DictConfig) -> Path:
    ...
    if output_name is None:
        ...
        output_name = f"brightness_tds_sweep_{timestamp}"
...
def setting_stem(guidance_scale: float, guidance_start: float) -> str:
    return f"scale{safe_value(guidance_scale)}_start{safe_value(guidance_start)}"
...
def save_plot(rows: list[dict], path: Path, original_mean: float, target_mean: float) -> None:
    ...
    ax.set_title("Brightness TDS guidance sweep")
    ax.grid(alpha=0.25)
...
def main(cfg: DictConfig) -> None:
    ...
    guidance_ramp = str(cfg.sweep.guidance_ramp)
    ...
    for guidance_start in [float(value) for value in cfg.sweep.guidance_starts]:
        for guidance_scale in [float(value) for value in cfg.sweep.guidance_scales]:
            ...
            stem = setting_stem(guidance_scale, guidance_start)
            ...
            if bool(cfg.sweep.save_samples):
                ...
                torch.save(
                    {
                        "samples": samples,
                        "sample_type": "brightness_tds_sweep_setting",
                        "guidance_scale": guidance_scale,
                        "guidance_start": guidance_start,
                        "guidance_ramp": guidance_ramp,
                        "target_marginal": {
    ...
    plot_path = sweep_dir / "brightness_mean_vs_guidance_scale.png"
    save_plot(
        rows,
        plot_path,
        original_mean=original_mean,
