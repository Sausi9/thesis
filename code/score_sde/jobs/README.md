...
```bash
RESUME_PATH=checkpoints/score_sde_cifar_<run_id>/latest.pt bsub < jobs/train_cifar.bsub
```

```bash
RESUME_PATH=checkpoints/score_sde_mnist_<run_id>/latest.pt bsub < jobs/train_mnist.bsub
```
