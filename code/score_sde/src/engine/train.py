...
from src.diffusion.sde import SDEConfig, VPSDE
...
def main():
    ...
    sde_cfg = dict(config["sde"])
    sde_type = str(sde_cfg["type"]).lower()
    if sde_type != "vp":
        raise NotImplementedError(f"Unsupported SDE type: {sde_type}")
    sde = VPSDE(SDEConfig(**sde_cfg))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for x0, _ in pbar:
            ...
            loss = ((score_pred * std + epsilon) ** 2).mean()
            loss.backward()
            optimizer.step()
